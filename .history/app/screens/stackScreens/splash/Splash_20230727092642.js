  ...
  const navi = async () => {
    try {
      ...
      if (token) {
        ...
        console.log(res.data.userData[0], 'device');
        dispatch({
          type: 'SET_USERINFO',
          payload: res.data.userData[0].subscribtion_expiration_date,
        });
        console.log(res.data?.userData, 'wwwwww');
        token && !res.data.userData.subscribtion_expiration_date
          ? navigation.replace('Subscribe')
          : token &&
            res.data.userData.subscribtion_expiration_dat &&
