...
export function Splash({navigation}) {
  ...
  const navi = async () => {
    try {
      ...
      token ? (res.data.userData.subscribtion_expiration_date && res.data.userData.subscribtion_expiration_date<=new Date() )
        ? navigation.replace('Subscribe') navigation.replace('TabNavigation')
    
        : !getStarted
        ? navigation.replace('GetStarted')
        : navigation.replace('SignIn'):null;
    } catch (err) {
