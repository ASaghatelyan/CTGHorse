...
export function Splash({navigation}) {
  ...
  const navi = async () => {
    try {
      ...
      if( token && !res.data.userData[0].subscribtion_expiration_date){
      ...
      }else if( res.data.userData[0].subscribtion_expiration_date &&
        moment(new Date()).isBefore(
          res.data.userData[0].subscribtion_expiration_date,
        )){
        ...
        token && !res.data.userData[0].subscribtion_expiration_date
          ...
          :  navigation.replace('Subscribe') ?navigation.replace('SignIn');
      }
    } catch (err) {
