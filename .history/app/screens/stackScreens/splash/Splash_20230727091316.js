...
export function Splash({navigation}) {
  ...
  const navi = async () => {
    try {
      ...
      console.log(moment(new Date()).isAfter(res.data.userData.subscribtion_expiration_dat));
      if (token) {
        let res = await axiosInstance.get(`/get-user-details`, {});
        let ddd = await axiosInstance.post('/device-token', data);
        console.log(res.data.userData.subscribtion_expiration_date
          , 'device');
        dispatch({
          type: 'SET_USERINFO',
          payload: res.data.userData[0],
        });
        console.log(  res.data?.userData,'wwwwww');
        token && !res.data.userData.subscribtion_expiration_date ?navigation.replace('Subscribe'):
        token && res.data.userData.subscribtion_expiration_dat  ? navigation.replace('TabNavigation')
        : !getStarted
