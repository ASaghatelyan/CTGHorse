...
export function HorseRegister({navigation}) {
  ...
  const [titleText, setTitleText] = useState(horsRegText[0].title);
  const [infoText, setInfoText] = useState(horsRegText[0].text);
  const renderPages = () => {
    switch (percentage) {
      case 25:
        return (
          () => {
            setInfoText(horsRegText[0].text);
            setTitleText(horsRegText[0].title)
          },
          (
            <HorseRegFirst
