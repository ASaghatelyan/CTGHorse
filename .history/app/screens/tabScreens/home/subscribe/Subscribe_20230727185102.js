...
export function Subscribe({navigation}) {
  ...
  const onSubscribe = async id => {
    if (method.length > 0) {
      ...
      axios.post(`https://5704.freelancedeveloper.site/api/subscribtion-to-plan/subscribtion-to-plan`,{formdata}).then((response) => {
      ...
      try {
       ...
       let formdata = new FormData();
        ...
        let ress = await axios.post(`https://5704.freelancedeveloper.site/api/subscribtion-to-plan/subscribtion-to-plan`, {formdata});
        // !firstLog
