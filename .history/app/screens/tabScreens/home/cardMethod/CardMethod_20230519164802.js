...
export   function CardMethod({navigation}) {const [choosed, setChoosed] = useState(0);
    const [payed, setPayed] = useState(false);
    const [modalVisible, setModalVisible] = useState(false);
    const [addCardModal, setAddCardModal] = useState(false);
    const [method, setMethod] = useState(route.params.data.sort(function (x, y) {
    ...
    let getDate = async (id, ind) => {
      try {
        ...
        setMethod(res.data.sort(function (x, y) {
          return x.default == 1 ? -1 : y.default == 1 ? 1 : 0;
        }));
        setLoad(false);
      } catch (error) {
