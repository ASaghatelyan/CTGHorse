...
export function HorseInfo({navigation, route}) {
  ...
  const generateThumbnail = async (videoUrl) => {
    // Use the video URL to fetch the thumbnail
    // You can use any library or service to fetch the thumbnail image,
    // for example, you can use the `react-native-thumbnail` library
    // to get the thumbnail.
    // For simplicity, let's assume you have a function `fetchThumbnailFromVideoURL`.
    const thumbnailImage = await fetchThumbnailFromVideoURL(videoUrl);
