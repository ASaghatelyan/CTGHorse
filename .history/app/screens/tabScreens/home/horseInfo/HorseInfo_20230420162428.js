...
export function HorseInfo({navigation, route}) {
  ...
  let getData = async () => {
    try {
      ...
      console.log(res.data[0][0],'res.data[0].favorite');
    } catch (error) {
  ...
  let onLikeDisLike = async num => {
    try {
      ...
      let res = await axiosInstance.post(`/favorite/${data?.[0]?.id}/${num}`);
      console.log(res,'dsafgsfd');
    } catch (error) {
      ...
      <View>
        <View style={styles.imageContainer}>
          ...
          <TouchableOpacity
            ...
            onPress={() => route.params.navigation.goBack(1)}>
