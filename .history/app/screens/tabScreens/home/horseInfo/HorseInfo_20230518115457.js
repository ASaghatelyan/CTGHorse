...
export function HorseInfo({navigation, route}) {
  ...
  let getOwnerData = async (id) => {
    try {
      let res = await axiosInstance.get(`/get-owner/${id}`);
      navigation.navigate('OwnerProfile', {owner:res.data[0],getOwnerData});
    } catch (error) {
      console.log(error.response);
    }
  };
