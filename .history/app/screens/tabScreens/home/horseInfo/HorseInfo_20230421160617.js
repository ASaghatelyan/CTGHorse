...
export function HorseInfo({navigation, route}) {
  ...
  let getData = async () => {
    try {
      ...
      console.log(res.data[0].medias[0].url, 'yghujkl;');
      setData(res.data[0]);
      setChoosed(res.data[0].medias[0].url);
      setLikeDisLike(res.data[0].favorite);
      setMore(res.data[1]);
      setStarCount(res.data[0].user.rate);
  ...
  let getOwnerData = async () => {
    try {
      let res = await axiosInstance.get(`/get-owner/${data?.user.id}`);
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={{marginTop: 16}}>
        <View style={styles.infoView}>
          ...
          <Text style={styles.titleName}>
            Reg N: {data?.registration_number}
          </Text>
          <Text style={styles.titleName}>
