...
export function HorseInfo({navigation, route}) {
  ...
  const [more, setMore] = useState([route.params?.item[1]]);
  ...
  let getData=async()=>{
    let res = await axiosInstance.get(`/get-horse/${item.id}`);
    console.log(res.data);
    owner
    ? navigation.navigate('ChangeHorseInfo', {item, index})
    : navigation.navigate('HorseInfo',{item:res.data[0],more:res.data[1], index})
    setData()
  }
  return (
