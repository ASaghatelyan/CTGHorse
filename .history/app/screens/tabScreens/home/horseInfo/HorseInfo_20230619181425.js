...
export function HorseInfo({navigation, route}) {
  ...
  const [acceptStatus,setAcceptStatus]=useState(route?.params?.item?.transaction?.status)
          ...
          <View style={styles.ownerTop}>
            <TouchableOpacity style={styles.ownerView} onPress={getOwnerData}>
              ...
              <View style={styles.gFlex}>
                ...
                <View>
                  <TouchableOpacity
                    onPress={() => data?.[0]?.user.phone && Linking.openURL(`tel:${data?.[0]?.user.phone}`)}>
                    <Image source={call} style={styles.callIc} />
                  </TouchableOpacity>
                  <TouchableOpacity onPress={()=>navigation.navigate('OpenChat',data?.[0]?.user)}>
