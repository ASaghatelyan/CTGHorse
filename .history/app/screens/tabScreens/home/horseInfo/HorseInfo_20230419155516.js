...
export function HorseInfo({navigation, route}) {
  ...
  const [data, setData] = useState([
    route.params?.item?.[0] ? route.params?.item?.[0] : route.params?.item,
  ]);
  const [more, setMore] = useState([
    route.params?.item?.[1] ? route.params?.item?.[1] : route.params?.item,
  ]);
  const [starCount, setStarCount] = useState(
    data?.[0]?.user?.rate ? data?.[0].user.rate : data?.user?.rate,
  );
  const [choosed, setChoosed] = useState(data?.[0].medias[0].url?data?.[0].medias[0].url:data?.[0].medias[0].url);
