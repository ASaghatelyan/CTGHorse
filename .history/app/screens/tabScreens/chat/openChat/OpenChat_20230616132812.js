...
export function OpenChat({navigation, route}) {
  ...
  const onSend = useCallback(async (newMessages = []) => {
    ...
    try {
      const data = {
        rec_id: userInfo.id,
        msg: newMessages.text,
            user:newMessages.user
      };
      const response = await axiosInstance.post('/messages', newMedatassages);
  ...
  useEffect(() => {
    loadChats();
    return () => {
      pusherInfo?.unsubscribe('my-channel');
    };
  }, []);
  ...
  const subscribeToPusher = async () => {
    ...
    channel.bind('my-event', d => {
      // setChatSecond(d);
      console.log(d, 'BBBBBBBBBBBBBBBB');
    });
