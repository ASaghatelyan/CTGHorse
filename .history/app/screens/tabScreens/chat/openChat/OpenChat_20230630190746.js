...
import {
  ...
  renderInputToolbar,
  renderSend,
} from 'app/components/chatComponent/inputToolbar/InputToolbar';
...
export function OpenChat({navigation, route}) {
  ...
  let isCloseToTop = ({layoutMeasurement, contentOffset, contentSize}) => {
    const paddingToTop = GlobalHeight(50);
    return (
  ...
  const loadChats = async () => {
    console.log(recId, 'ressssss');
    try {
      const response = await axiosInstance.post('/fetchMessages/' + recId);
  ...
  useEffect(() => {
    ...
    return async () => {
      ...
      await pusherInfo?.unsubscribe(`my-channel-${userInfo.id}-${recId}`);
      route.params.loadChats();
    };
  }, []);
  ...
  const subscribeToPusher = async () => {
    ...
    let pusher = new Pusher('a1ddbc653c1b9f25b042', {
      cluster: 'ap2',
      // authEndpoint:
      //   'https://5704.freelancedeveloper.site/api/broadcasting/auth',
      encrypted: true,
    });
    var channel = pusher.subscribe(`my-channel-${recId}-${userInfo.id}`);
  ...
  return (
    <KeyboardAvoidingView
      ...
      <ChatHeader item={route.params.item} setStopPlay={setStopPlay}/>
      <GiftedChat
        textInputProps={{
          paddingTop: 8,
        }}
        ...
        renderBubble={props => {
          return (
            <Bubble
              ...
              renderMessageAudio={() => (
                <AudioPlayer url={props.currentMessage.audio} onStop={stopPlay} />
              )}
            />
          );
        }}
        ...
        renderFooter={props => (
          <View style={{height: typing ? GlobalHeight(60) : GlobalHeight(40)}}>
            {typing && (
              <TypingAnimation
                dotColor="#E9A13A"
                dotMargin={5}
