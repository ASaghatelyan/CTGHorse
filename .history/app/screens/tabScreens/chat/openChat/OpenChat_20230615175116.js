...
import {  Pusher,
  ...
  PusherChannel,
  PusherEvent,
  PusherAuthorizerResult,} from '@pusher/pusher-websocket-react-native';
  ...
  const subscribeToPusher = async () => {
    try {
      ...
      let myChannel = await pusher.subscribe({
        channelName: 'private-chat-' + route.params.id,
        onEvent: (event: PusherEvent) => {
          console.log(`onEvent: ${event}`);
        },
      });

      console.log(myChannel, 'fffsfas');
  
  
      // var channel = pusher.subscribe('private-chat-' + userInfo.id);
      // channel.bind('messageSend', d => {
      //   setChatSecond(d);
      //   console.log(d, 'fffffffggfwsdb');
      // });
    } catch (error) {}
  };
  ...
  const trigger = async () => {
    try {
      ...
      await pusher.trigger(
        new PusherEvent(  'private-chat-' + route.params.id, eventName, data: eventData  )
      );
    } catch (e) {
      log('ERROR: ' + e);
    }
