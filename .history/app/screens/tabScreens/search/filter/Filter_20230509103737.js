...
export function Filter({navigation, route}) {
  ...
  // console.log(_.includes(filterData[type], item.title));
  //   let arr =item
  //   item.type= type  
  //   let filtered = _.includes(filterData[type], item.title);
  //   console.log(filtered);
  //   if (!filtered) {
  //     filterData[type].push(item);
  //   } else {
  //     _.remove(filterData[type], el => {
  //       console.log( el ,item,'ddfgbwefdd');
  //       return el === item;
  //     });
  //   }
  ...
  let onDeletFilter = it => {
    let filtered = _.includes(filterData[it.type], it);
    if (!filtered) {
      filterData[it.type].push(ititem);
    } else {
      _.remove(filterData[it.type], el => {
        return el === item;
      });
      setFilterData({...filterData});
    }
    ...
    <View style={styles.content}>
      ...
      <View style={styles.itemsView}>
        ...
        <FilterItem
          ...
          reset={JSON.parse(JSON.stringify(color))}
        />
        <FilterItem
          text="Sex"
          addFilter={addFilter}
