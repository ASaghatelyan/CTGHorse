import {
  View,
  Text,
  SafeAreaView,
  ScrollView,
  Image,
  TouchableOpacity,
  Platform,
  Linking,
} from 'react-native';
import React, {useEffect, useState} from 'react';
import {FilterItem, GButton, HeaderNavi} from 'app/components';
import {styles} from './style';
import {
  breed,
  sex,
  height,
  weight,
  color,
  price,
  training,
  discipline,
} from 'app/constant/filterData';
import deleteItem from 'app/assets/img/delete.png';
import * as _ from 'lodash';
import {useSelector, useDispatch} from 'react-redux';
import axiosInstance from 'app/networking/api';

export function Filter({navigation, route}) {
  let dispatch = useDispatch();
  let useFilterData = useSelector(store => store.filterData);
  let useFilerItems = useSelector(store => store.filterItem);
  console.log(useFilerItems);
  let [colorH, setColorH] = useState(
    useFilerItems.Color
      ? useFilerItems.Color
      : JSON.parse(JSON.stringify(color)),
  );
  let [sexH, setSexH] = useState(
    useFilerItems.Sex ? useFilerItems.Sex : JSON.parse(JSON.stringify(sex)),
  );
  let [heightH, setHeightH] = useState(
    useFilerItems.Height
      ? useFilerItems.Height
      : JSON.parse(JSON.stringify(height)),
  );
  let [weightH, setWeightH] = useState(
    useFilerItems.Weight
      ? useFilerItems.Weight
      : JSON.parse(JSON.stringify(weight)),
  );
  let [priceH, setPriceH] = useState(useFilerItems.Earnings ?useFilerItems.Earnings:JSON.parse(JSON.stringify(price)));
  let [trainingH, setTrainingH] = useState(
    JSON.parse(JSON.stringify(training)),
  );
  let [filterData, setFilterData] = useState(useFilterData);
  let [disciplineH, setDisciplineH] = useState(
    JSON.parse(JSON.stringify(discipline)),
  );
  let [breedH, setbreedH] = useState(JSON.parse(JSON.stringify(breed)));
  let [allData, setAllData] = useState([]);

  // let openMap = () => {
  //   Platform.OS === 'ios'
  //     ? Linking.openURL('http://maps.apple.com/maps?daddr=20 Cooper Square, New York, NY 10003, USA')
  //     : Linking.openURL('http://maps.google.com/maps?daddr=20 Cooper Square, New York, NY 10003, USA');
  // };

  useEffect(() => {
    let arr1 = [];
    let arr = Object.values(filterData);
    let arrConcat = [];
    for (let index = 0; index < arr.length; index++) {
      for (let j = 0; j < arr[index].length; j++) {
        arrConcat.push(arr[index][j]);
      }
    }
    setAllData([...arrConcat]);
  }, [filterData]);

  let addFilter = (item, type) => {
    let filtered = _.includes(filterData[type], item);
    if (!filtered) {
      filterData[type].push(item);
    } else {
      _.remove(filterData[type], el => {
        return el === item;
      });
    }
    setFilterData({...filterData});
  };

  return (
    <View style={styles.content}>
      <SafeAreaView />
      <HeaderNavi
        leftBtn="Filter"
        rightBtn="Clear all"
        leftOnPress={async () => {
          try {
            dispatch({
              type: 'RESET_FILTERDATA',
            });
            let res = await axiosInstance.post(`/filter`, {
              arr: [],
            });
            route.params.onConfirmFilters(res.data, 0, allData);
            navigation.goBack();
          } catch (error) {
            console.log(error.response);
          }
        }}
        rightOnPress={() => {
          setFilterData({
            color: [],
            sex: [],
            height: [],
            weight: [],
            earnings: [],
            breed: [],
            training: [],
            discipline: [],
          });
          dispatch({
            type: 'RESET_FILTERDATA',
          });
        }}
      />
      <View style={styles.itemsView}>
        <ScrollView
          horizontal={true}
          showsHorizontalScrollIndicator={false}
          contentContainerStyle={{
            flexGrow: 1,
            flexDirection: 'row',
            paddingHorizontal: 15,
          }}>
          {allData.map((item, index) => {
            return (
              <View style={styles.filtered}>
                <TouchableOpacity style={styles.delView}>
                  <Image source={deleteItem} style={styles.del} />
                </TouchableOpacity>
                <Text numberOfLines={1} style={styles.choosedText}>
                  {item}
                </Text>
              </View>
            );
          })}
        </ScrollView>
        <FilterItem
          text="Color"
          addFilter={addFilter}
          info={colorH}
          onReset={() => {
            setColorH(JSON.parse(JSON.stringify(color)));
          }}
          navigation={navigation}
          saveInfo={() => {
            dispatch({
              type: 'SET_FILTERITEM',
              payload: {Color: colorH},
            });
          }}
          data={JSON.parse(JSON.stringify(colorH))}
        />
        <FilterItem
          text="Sex"
          addFilter={addFilter}
          info={sexH}
          navigation={navigation}
          onReset={() => {
            setSexH(JSON.parse(JSON.stringify(sex)));
          }}
          saveInfo={d => {
            dispatch({
              type: 'SET_FILTERITEM',
              payload: {Sex: sexH},
            });
          }}
        />
        <FilterItem
          addFilter={addFilter}
          text="Height"
          info={heightH}
          navigation={navigation}
          onReset={() => {
            setHeightH(JSON.parse(JSON.stringify(height)));
          }}
          saveInfo={d => {
            dispatch({
              type: 'SET_FILTERITEM',
              payload: {Height: heightH},
            });
          }}
        />
        <FilterItem
          addFilter={addFilter}
          text="Weight"
          info={weightH}
          navigation={navigation}
          onReset={() => {
            setWeightH(JSON.parse(JSON.stringify(weight)));
          }}
          saveInfo={d => {
            dispatch({
              type: 'SET_FILTERITEM',
              payload: {Weight: weightH},
            });
          }}
        />
        <FilterItem
          text="Earnings"
          addFilter={addFilter}
          info={priceH}
          navigation={navigation}
          onReset={() => {
            setPriceH(JSON.parse(JSON.stringify(price)));
          }}
          saveInfo={d => {
            dispatch({
              type: 'SET_FILTERITEM',
              payload: {Earnings: priceH},
            });
          }}
        />
        <FilterItem
          addFilter={addFilter}
          text="Breed"
          info={breedH}
          navigation={navigation}
          onReset={() => {
            setbreedH(JSON.parse(JSON.stringify(breed)));
          }}
          saveInfo={d => {
            dispatch({
              type: 'SET_FILTERITEM',
              payload: {Breed: breedH},
            });
          }}
        />
        <FilterItem
          addFilter={addFilter}
          text="Training"
          info={trainingH}
          navigation={navigation}
          onReset={() => {
            setTrainingH(JSON.parse(JSON.stringify(training)));
          }}
          saveInfo={d => {
            dispatch({
              type: 'SET_FILTERITEM',
              payload: {Training: trainingH},
            });
          }}
        />
        <FilterItem
          addFilter={addFilter}
          text="Discipline"
          info={disciplineH}
          navigation={navigation}
          onReset={() => {
            setHeightH(JSON.parse(JSON.stringify(discipline)));
          }}
          saveInfo={d => {
            dispatch({
              type: 'SET_FILTERITEM',
              payload: {Discipline: disciplineH},
            });
          }}
        />
      </View>
      <View style={styles.bottomView}>
        <GButton
          btnName="View Results"
          handelMove={async () => {
            try {
              dispatch({
                type: 'SET_FILTERDATA',
                payload: filterData,
              });
              let res = await axiosInstance.post(`/filter`, {
                arr: allData,
              });
              route.params.onConfirmFilters(res.data, allData.length, allData);
              navigation.goBack();
            } catch (error) {
              console.log(error.response);
            }
          }}
        />
      </View>
    </View>
  );
}
