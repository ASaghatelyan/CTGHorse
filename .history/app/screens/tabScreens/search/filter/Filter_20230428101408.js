...
export function Filter({navigation, route}) {
  ...
  let onDeletFilter = (it)=>{
    console.log(it);
    console.log(Object.keys(filterData).find(key => filterData[key] === it));
      Object.keys(filterData).find(key => filterData[key] === it);
          ...
          {allData.map((item, index) => {
            return (
              <View style={styles.filtered}>
                <TouchableOpacity style={styles.delView} onPress={()=>onDeletFilter(item)}>
