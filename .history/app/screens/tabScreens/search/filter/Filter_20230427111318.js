...
export function Filter({navigation, route}) {
  ...
  console.log(useFilerItems);
  let [colorH, setColorH] = useState(useFilerItems.Color?useFilerItems.Color: JSON.parse(JSON.stringify(color)));
  let [sexH, setSexH] = useState(useFilerItems.Sex ?useFilerItems.Sex :JSON.parse(JSON.stringify(sex)));
  let [heightH, setHeightH] = useState(useFilerItems.Height?useFilerItems.Height JSON.parse(JSON.stringify(height)));
  ...
  // let openMap = () => {
  //   Platform.OS === 'ios'
  //     ? Linking.openURL('http://maps.apple.com/maps?daddr=20 Cooper Square, New York, NY 10003, USA')
  //     : Linking.openURL('http://maps.google.com/maps?daddr=20 Cooper Square, New York, NY 10003, USA');
  // };
      ...
      <HeaderNavi
        ...
        leftOnPress={async () => {
          try {
            ...
            route.params.onConfirmFilters(res.data, 0, allData);
            navigation.goBack();
          } catch (error) {
