...
export function SearchScreen(props) {
  ...
  return (
    <View style={{flex: 1,backgroundColor:'#F5F5F5'}}>
      ...
      <ScrollView ref={scrollRef} contentContainerStyle={styles.itemContainer} showsVerticalScrollIndicator={false}>
        {data.map((item, index) => {
          return (
            <HorseItemComponent
              key={item.id}
      ...
      <SearchModal isVisible={searchModal} onClose={() => setSearchModal(!searchModal)}/>
    </View>
  );
}
