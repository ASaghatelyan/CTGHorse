...
export function TransactionFilter({navigation, route}) {
  ...
  const status = ['', 'completed', 'incompleted'];
  return (
            ...
            {status.map((item, index) => {
              return (
                <TouchableOpacity
                  ...
                  <Image source={choosed === item ? off : on}  style={styles.onOff}/>
                  <Text style={styles.statusText}>{item==='' ?:item}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
        ...
        <GButton
          ...
          handelMove={() => {
            if (startDate && endDate) {
              route.params(startDate, endDate,choosed);
              navigation.goBack();
            }
          }}
        />
