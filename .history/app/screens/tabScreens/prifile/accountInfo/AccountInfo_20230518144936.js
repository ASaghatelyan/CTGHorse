...
export function AccountInfo({navigation}) {
  ...
  let setAsDefault = async it => {
    try {
      setLoad(true);
      await axiosInstance.post(`/delete-card//make-card-default/{cardId}${it.item.db_id}`);
      setMethod([...method.filter((item, index) => index !== it.index)]);
      setLoad(false);
    } catch (error) {
      setLoad(false);
  ...
  const renderItem = ({item, index}, onClick) => {
    ...
    const renderRightActions = (progress, dragX, onClick) => {
      return (
        <TouchableOpacity style={styles.deleteChatView} onPress={()=>deletCard(v)}>
    ...
    const rendeLeftActions = (progress, dragX, onClick) => {
      return (
        <TouchableOpacity style={styles.deleteChatView} onPress={()=>console.log('left')}>
          <Image source={deleteChat} style={styles.deleteChat} />
        </TouchableOpacity>
      );
    };
