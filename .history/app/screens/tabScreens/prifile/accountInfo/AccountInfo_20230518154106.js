...
export function AccountInfo({navigation}) {
  ...
  let getDate = async () => {
    try {
      ...
      console.log(res);
      setMethod(res.data.sort(function(x,y){ return x.default == 1 ? -1 : y.default == 1 ? 1 : 0; }));
  ...
  const renderItem = ({item, index}, onClick) => {
    ...
    const rendeLeftActions = (progress, dragX, onClick) => {
      return (
        <TouchableOpacity
          style={styles.deleteChatView}
          onPress={() => setAsDefault(item.db_id)}>
          <Image source={deleteChat} style={styles.deleteChat} />
        </TouchableOpacity>
      );
    };
    ...
    return (
      <Swipeable
        renderRightActions={(progress, dragX) =>
          renderRightActions(progress, dragX, onClick,closeRow(index))
        }
