...
export function AccountInfo({navigation}) {
  ...
  let deletCard = async id => {
    try {
      ...
      setMethod([...method.filter((item, index) => item.id !== id)]);
      setLoad(false);
    } catch (error) {
      setLoad(false);
      console.log(error.response);
  ...
  let setAsDefault = async id => {
    try {
      setLoad(true);
     let re= await axiosInstance.post(`/make-card-default/${id}`);
     console.log(r);
      getDate()
      setLoad(false);
    } catch (error) {
  ...
  const renderItem = ({item, index}, onClick) => {
    ...
    const renderRightActions = (progress, dragX, onClick) => {
      return (
        <TouchableOpacity style={styles.deleteChatView} onPress={()=>deletCard(item.db_id)}>
          <Image source={deleteChat} style={styles.deleteChat} />
        </TouchableOpacity>
      );
    };

    const rendeLeftActions = (progress, dragX, onClick) => {
      return (
        <TouchableOpacity style={styles.deleteChatView} onPress={()=>setAsDefault(item.db_id)}>
          <Image source={deleteChat} style={styles.deleteChat} />
        </TouchableOpacity>
      );
    };
    ...
    return (
      <Swipeable
        ...
        renderLeftActions={(progress, dragX) =>
          rendeLeftActions(progress, dragX, onClick)
        }
        onSwipeableOpen={() => {
          closeRow(index);
        }}
        ...
        <View style={styles.addItem}>
         ...
         {item.default ===1 && <View style={styles.statementView}>
