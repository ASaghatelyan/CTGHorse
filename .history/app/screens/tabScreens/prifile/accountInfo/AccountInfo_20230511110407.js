...
export function AccountInfo({navigation}) {
  ...
  let deletCard = async (cardId) => {
    try {
      setLoad(true);
      let res = await axiosInstance.get(`/delete-card/${cardId} `);
 method.filter((item,index)=>index!==ind)
      setLoad(false);
    } catch (error) {
      setLoad(false);
      console.log(error.response);
    ...
    <View style={styles.content}>
      ...
      <ScrollView
        ...
        <View>
         ...
         return   <TouchableOpacity style={styles.addItem}>
              <View style={styles.addLeftView}>
              <PaymentIcon  style={{marginRight:10}} type={item.brand.replace(' ', '-').toLowerCase()} width={40} height='20%' /> 
                <View>
                  <Text style={styles.saveingText}>Saving Account </Text>
                  <Text style={styles.saveNumText}>91212192291221</Text>
                  <Text style={styles.saveNumText}>  ***** ***** **** {item.last4}</Text>
                </View>
              </View>
