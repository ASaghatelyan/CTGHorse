...
export function ChangeHorseInfo({navigation, route}) {
  let userHorseInf = useSelector(
    state => state.userHorseInf.horses[route.params.index],
  );
  const [load, setLoad] = useState(false);
  const [err, setErr] = useState('');
  const [errModal, setErrModal] = useState(false);
  const [genImg, setGenImg] = useState(userHorseInf.medias[0].url);
  const [modalVisible, setModalVisible] = useState(false);
  const [location, setLocation] = useState(
    userHorseInf.location ? userHorseInf.location : '',
  );
  const [changePicModal, setChangePicModal] = useState(false);
  let doc = [
    ...userHorseInf.medias.filter((item, index) => item.type === 'documents'),
  ];
  const [name, setName] = useState(userHorseInf.name ? userHorseInf.name : '');
  const [date, setDate] = useState(
    userHorseInf.date_of_birth ? userHorseInf.date_of_birth : '',
  );

  const [disciplineH, setDisciplineH] = useState(
    userHorseInf.discipline ? userHorseInf.discipline : '',
  );
  const [breedH, setBreedH] = useState(userHorseInf.breed ? userHorseInf.breed : '');
  const [sexH, setSexH] = useState(userHorseInf.sex ? userHorseInf.sex : '');
  const [heightH, setHeightH] = useState(
    userHorseInf.height ? userHorseInf.height : '',
  );
  const [weightH, setWeightH] = useState(
    userHorseInf.weight ? userHorseInf.weight : '',
  );
  const [colorH, setColorH] = useState(userHorseInf.color ? userHorseInf.color : '');
  const [trainingH, setTrianingH] = useState(
    userHorseInf.training ? userHorseInf.training : '',
  );
  const [priceH, setPriceH] = useState(userHorseInf.price ? userHorseInf.price : '');
  const [conditionH, setConditionH] = useState(
    userHorseInf.condition ? userHorseInf.condition : '',
  );
  const [proEarnings, setProEarnings] = useState(
    userHorseInf.produced_earnings ? userHorseInf.produced_earnings : '',
  );
  const [earnings, setEarnings] = useState(
    userHorseInf.earnings ? userHorseInf.earnings : '',
  );
  const [sir, setSir] = useState(userHorseInf.sire ? userHorseInf.sire : '');
  const [dam, setDam] = useState(userHorseInf.dam ? userHorseInf.dam : '');
  const [description, setdDescription] = useState(
    userHorseInf.description ? userHorseInf.description : '',
  );

  console.log(userHorseInf);

  let editInfo = async () => {
    try {
      setLoad(true);
      let formData = new FormData();
      formData.append('id', userHorseInf.id);
      formData.append('name', name);
      formData.append('registration_number', userHorseInf.registration_number);
      ...
      // userHorseInf?.videoData?.map((item, index) => {
      //   if (item) {
      ...
      //   }
      // });
      // userHorseInf?.doc?.map((item, index) => {
      //   if (item) {
      ...
      //   }
      // });
      // userHorseInf?.allImg?.map((item, index) => {
      ...
      // await userHorseInf.horses.push(res.data.newHorse[0]);
      // dispatch({
      //   type: 'SET_USERINFO',
      //   payload: user,
      // });
    ...
    <View style={styles.content}>
      ...
      <ScrollView
        ...
        <ScrollView
          ...
          {userHorseInf.medias.map((item, index) => {
            if (item.type === 'photos') {
              return (
                <ImageBackground
                  source={{uri: item.url}}
        ...
        <InputHorseReg
          ...
          value={userHorseInf.registration_number}
          colorT={{color: '#190C04'}}
          editable={false}
        />
        <View style={styles.gView}>
