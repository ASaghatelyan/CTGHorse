...
export function ChangeHorseInfo({navigation, route}) {
  ...
  let deletImg = async (id, ind) => {
    try {
      id && (await axiosInstance.delete(`/delete-media/${id}`));
      let arr = img.filter((item, index) => {
        return ind !== index;
      }); 
      setImg(arr);
  ...
  let changeMainPic = async (media_id) => {
    try {
      await axiosInstance.put(`/change-main-photo/${userHorseInfo.id}/${media_id}`);
  ...
  return (
    <View style={styles.content}>
      ...
      <ScrollView
        ...
        <ImageBackground
          source={{uri: img?.filter((item, index) => {
            return item.main === 1;
          })[0]?.url}}
          style={styles.topimg}
          resizeMode="contain"
      ...
      <ChangePhotoModal
        ...
        img={JSON.parse(JSON.stringify(userHorseInfo?.medias))}
        genImg={img}
