...
export function ChangeHorseInfo({navigation, route}) {
  ...
  let userHorses = useSelector(
    state => state.userInfo.horses ,
  );
  ...
  const [name, setName] = useState(userHorseInfo.name ? userHorseInfo.name : '');
  ...
  const [breedH, setBreedH] = useState(userHorseInfo.breed ? userHorseInfo.breed : '');
  ...
  const [colorH, setColorH] = useState(userHorseInfo.color ? userHorseInfo.color : '');
  const [trainingH, setTrianingH] = useState(
    userHorseInfo.training ? userHorseInfo.training : '',
  );
  const [priceH, setPriceH] = useState(userHorseInfo.price ? userHorseInfo.price : '');
  ...
  let getinfo = async () => {
    try {
      ...
      console.log(res.data.userData[0],'resres');
      dispatch({
        type: 'SET_USERINFO',
  ...
  let editInfo = async () => {
    try {
      ...
      // await userHorseInfo.horses.push(res.data.newHorse[0]);
      // dispatch({
      //   type: 'SET_USERINFO',
      //   payload: user,
      // });
