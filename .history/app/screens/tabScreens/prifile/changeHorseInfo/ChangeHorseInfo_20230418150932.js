...
export function ChangeHorseInfo({navigation, route}) {
  let userInfo = useSelector(state => state.userInfo.horses[route.params.index]);
  console.log(userInfo.medias ,'fgsnv');
  const [genImg, setGenImg] = useState(
    userInfo.medias[0].url
  );
      ...
      <ScrollView
        ...
        <ScrollView
          ...
          {userInfo.medias.map((item, index) => {
           if(item.type==='photos' || item.type==='videos'){ return (
              <ImageBackground source={{uri:item.url}} style={styles.horseIc}>
                <TouchableOpacity>
