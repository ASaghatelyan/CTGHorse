...
export function ChangeHorseInfo({navigation, route}) {
  let userH = useSelector(
    state => state.userH.horses[route.params.index],
  );
  const [load, setLoad] = useState(false);
  const [err, setErr] = useState('');
  const [errModal, setErrModal] = useState(false);
  const [genImg, setGenImg] = useState(userH.medias[0].url);
  const [modalVisible, setModalVisible] = useState(false);
  const [location, setLocation] = useState(
    userH.location ? userH.location : '',
  );
  const [changePicModal, setChangePicModal] = useState(false);
  let doc = [
    ...userH.medias.filter((item, index) => item.type === 'documents'),
  ];
  const [name, setName] = useState(userH.name ? userH.name : '');
  const [date, setDate] = useState(
    userH.date_of_birth ? userH.date_of_birth : '',
  );

  const [disciplineH, setDisciplineH] = useState(
    userH.discipline ? userH.discipline : '',
  );
  const [breedH, setBreedH] = useState(userH.breed ? userH.breed : '');
  const [sexH, setSexH] = useState(userH.sex ? userH.sex : '');
  const [heightH, setHeightH] = useState(
    userH.height ? userH.height : '',
  );
  const [weightH, setWeightH] = useState(
    userH.weight ? userH.weight : '',
  );
  const [colorH, setColorH] = useState(userH.color ? userH.color : '');
  const [trainingH, setTrianingH] = useState(
    userH.training ? userH.training : '',
  );
  const [priceH, setPriceH] = useState(userH.price ? userH.price : '');
  const [conditionH, setConditionH] = useState(
    userH.condition ? userH.condition : '',
  );
  const [proEarnings, setProEarnings] = useState(
    userH.produced_earnings ? userH.produced_earnings : '',
  );
  const [earnings, setEarnings] = useState(
    userH.earnings ? userH.earnings : '',
  );
  const [sir, setSir] = useState(userH.sire ? userH.sire : '');
  const [dam, setDam] = useState(userH.dam ? userH.dam : '');
  const [description, setdDescription] = useState(
    userH.description ? userH.description : '',
  );
  ...
  let editInfo = async () => {
    try {
      ...
      formData.append('id', userH.id);
      formData.append('name', name);
      formData.append('registration_number', userH.registration_number);
      ...
      // userH?.videoData?.map((item, index) => {
      //   if (item) {
      ...
      // await userH.horses.push(res.data.newHorse[0]);
      // dispatch({
      //   type: 'SET_USERINFO',
      //   payload: user,
      // });
    ...
    <View style={styles.content}>
      ...
      <ScrollView
        ...
        <InputHorseReg
          ...
          value={userH.registration_number}
          colorT={{color: '#190C04'}}
          editable={false}
        />
        <View style={styles.gView}>
