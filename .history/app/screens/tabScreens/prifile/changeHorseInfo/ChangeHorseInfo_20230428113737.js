...
export function ChangeHorseInfo({navigation, route}) {
  let userHorse = useSelector(
    state => state.userHorse.horses[route.params.index],
  );
  const [load, setLoad] = useState(false);
  const [err, setErr] = useState('');
  const [errModal, setErrModal] = useState(false);
  const [genImg, setGenImg] = useState(userHorse.medias[0].url);
  const [modalVisible, setModalVisible] = useState(false);
  const [location, setLocation] = useState(
    userHorse.location ? userHorse.location : '',
  );
  const [changePicModal, setChangePicModal] = useState(false);
  let doc = [
    ...userHorse.medias.filter((item, index) => item.type === 'documents'),
  ];
  const [name, setName] = useState(userHorse.name ? userHorse.name : '');
  const [date, setDate] = useState(
    userHorse.date_of_birth ? userHorse.date_of_birth : '',
  );

  const [disciplineH, setDisciplineH] = useState(
    userHorse.discipline ? userHorse.discipline : '',
  );
  const [breedH, setBreedH] = useState(userHorse.breed ? userHorse.breed : '');
  const [sexH, setSexH] = useState(userHorse.sex ? userHorse.sex : '');
  const [heightH, setHeightH] = useState(
    userHorse.height ? userHorse.height : '',
  );
  const [weightH, setWeightH] = useState(
    userHorse.weight ? userHorse.weight : '',
  );
  const [colorH, setColorH] = useState(userHorse.color ? userHorse.color : '');
  const [trainingH, setTrianingH] = useState(
    userHorse.training ? userHorse.training : '',
  );
  const [priceH, setPriceH] = useState(userHorse.price ? userHorse.price : '');
  const [conditionH, setConditionH] = useState(
    userHorse.condition ? userHorse.condition : '',
  );
  const [proEarnings, setProEarnings] = useState(
    userHorse.produced_earnings ? userHorse.produced_earnings : '',
  );
  const [earnings, setEarnings] = useState(
    userHorse.earnings ? userHorse.earnings : '',
  );
  const [sir, setSir] = useState(userHorse.sire ? userHorse.sire : '');
  const [dam, setDam] = useState(userHorse.dam ? userHorse.dam : '');
  const [description, setdDescription] = useState(
    userHorse.description ? userHorse.description : '',
  );

  console.log(userHorse);

  let editInfo = async () => {
    try {
      setLoad(true);
      let formData = new FormData();
      formData.append('id', userHorse.id);
      formData.append('name', name);
      formData.append('registration_number', userHorse.registration_number);
      ...
      // userHorse?.videoData?.map((item, index) => {
      //   if (item) {
      ...
      //   }
      // });
      // userHorse?.allImg?.map((item, index) => {
      ...
      // await userHorse.horses.push(res.data.newHorse[0]);
      // dispatch({
      //   type: 'SET_USERINFO',
      //   payload: user,
      // });
    ...
    <View style={styles.content}>
      ...
      <ScrollView
        ...
        <ScrollView
          ...
          {userHorse.medias.map((item, index) => {
            if (item.type === 'photos') {
              return (
                <ImageBackground
                  source={{uri: item.url}}
        ...
        <InputHorseReg
          ...
          value={userHorse.registration_number}
          colorT={{color: '#190C04'}}
          editable={false}
        />
        <View style={styles.gView}>
