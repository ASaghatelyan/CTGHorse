...
export function EditProfile({navigation}) {
  ...
  console.log( address ===null ? '' :address ,'g', (userInfo?.address === 'null' ? '' : userInfo?.address),'sssssss',
  `${phone}` ,'dd',(userInfo?.phone === 'null' ? '' : userInfo?.phone));

  const chekChanges = () => {
    if (
      name === userInfo.name &&
      (address ===null ? '' :address) === (userInfo?.address === null ? '' : userInfo?.address) &&
