...
export function ChangePhotoModal({
  ...
  let [d, setD] = useState(genImg);
  useLayoutEffect(() => {
    let genImage = genImg?.filter((item, index) => {
      return item.main === 1;
    });
    console.log(genImage[0]);
    setMainImg(genImage[0].url);
  }, [genImg]);

  let changePic = (url, id) => {
    let arr = [...genImg];
    setMainImg(url);

    console.log(
      arr.map((item, index) => {
        console.log(item,'ddddwww');
        if (id === item.id) {
        ...
        {img &&
          img?.map((item, index) => {
            if (item.types === 'photos') {
              return (
                <TouchableOpacity
                  onPress={() => changePic(item.url, item.id)}
