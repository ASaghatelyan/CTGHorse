  ...
  let addCard = async () => {
    try {
      if (cardNum && cvv && month && year) {
        ...
        console.log(cardData.error,'ddddd');
        if(!cardData.error){
          console.log('wwwwww');
          ...
          }else{setErr(cardData.error.message)}
          setLoad(false);
...
let _onChange = form => console.log(form);

var str = '12/24';
 
console.log(str.slice(0, str.lastIndexOf('/') + 1),str.slice(str.lastIndexOf('/') + 1));
  return (
