...
export function HorseRegFirst({onNextPrev, percentage, navigation, setIndex}) {
  ...
  let stose = useSelector(state => state.horseRegister);
 
  const [active, setActive] = useState(false);
  const [text, setText] = React.useState();
  React.useEffect(() => {
    LayoutAnimation.spring(()=> setText(
      'Please allow time for large data files to load prior to advancing to next screen.'
    ));
  }, [active]);
