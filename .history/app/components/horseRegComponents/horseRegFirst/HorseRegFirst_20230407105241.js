...
export function HorseRegFirst({navigation,percentage,setPercentage ,setData,data}) {
  let dispatch =useDispatch()
  const [generalImg, setGeneralImg] = useState(data.generalImg?data.generalImg:'');
  const [image, setImage] = useState(data.image?data.image:['', '', '']);
  const [videoData, setVideoData] = useState(data.videoData?data.videoData:['', '']);
  ...
  return (
    ...
    <BottomBtn
        ...
        onChangeR={() =>
          percentage !== 100
            ? (setPercentage(prev => prev + 25),setData([{generalImg,image,videoData}]))
            :( setData([generalImg,image,videoData]),props.navigation.replace('HorseRegComplite'))
        }
      />
    </View>
