...
export function HorseRegFourth({onNextPrev, percentage, navigation}) {
  ...
  let next = async () => {
    try {
      ...
      formData.append('registration_number', 16732423024);
      ...
      // formData.append(`videos[]`, {
      //   name: store?.videoData[0].fileName,
      //   uri: store?.videoData[0].uri,
      //   type: 'video/mp4',
      // });
      ...
      console.log([store.genImg, ...store?.pictures]);
      let a=...store?.pictures
      [store.genImg,store?.pictures.length>0  ...store?.pictures].map((item, index) => {
