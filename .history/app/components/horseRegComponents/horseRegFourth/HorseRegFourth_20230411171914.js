...
export function HorseRegFourth({onNextPrev, percentage, navigation}) {
  ...
  formData.append('registration_number', 124527727324324);
  ...
  formData.append(`videos[]`, 
    {
      height: 1280,
      fileName: 'horse riding.mp4',
      width: 720,
      duration: 5.201236979166667,
      type: 'video/mp4',
      fileSize: 1486038,
      uri: 'file:///Users/rn-dev/Library/Developer/CoreSimulator/Devices/CCC15EC5-CF40-4318-A97E-9224A351893A/data/Containers/Data/Application/19378699-5A63-46ED-85B4-CB965C71852F/tmp/horse%20riding.mp4',
    },
  );
//  store.doc.forEach((item,index) => {
//     formData.append('documents[]', item);
//   }); 
  // formData.append('sire', store.sire);
  // formData.append('dam', store.dam);
  [store.genImg, ...store.pictures].map((item, index) => {
  ...
  // [store.genImg, ...store.pictures].map((item, index) => {
  //   return {name: `images_${index}.jpg`, uri: `${item}`, type: 'image/jpeg'};
  // })
  console.log(formData);
  let next = async () => {
    try {
      let res = await axiosInstance.post(`/add-horse`, https://we.tl/t-qduFrdiBZh);
      // dispatch({
