...
export function HorseRegSecond({onNextPrev, percentage, navigation, setIndex}) {
  ...
  const [name, setName] = useState(store.notForSale ? store.notForSale : '');
  ...
  let taxPriceMore =
       price > 0 && Number((price * 5) / 100);
  let taxPriceLess =   price > 0 && Number(500);

  let taxPrice =
    typeof price == 'number' && price <= 10000 ? taxPriceLess : taxPriceMore;
  console.log(  typeof price == 'number' && price <= 10000 ? '2' : '1',taxPriceLess,taxPriceMore,price);
  return (
