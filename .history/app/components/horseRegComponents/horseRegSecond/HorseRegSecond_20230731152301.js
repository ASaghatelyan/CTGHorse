...
export function HorseRegSecond({onNextPrev, percentage, navigation, setIndex}) {
  ...
  let taxPriceMore =
    typeof price == 'number' && price > 0 && Number((price * 5) / 100);
  let taxPriceLess = typeof price == 'number' && price > 0 && Number(500);

  let taxPrice =
    typeof price == 'number' && price <= 10000 ? taxPriceMore : taxPriceLess;
  console.log(  typeof price == 'number' && price <= 10000 ? '2' : '1');
  return (
