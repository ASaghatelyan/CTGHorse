...
export function AudioRecorder({sendVoice,setStartRec}) {
  const [recStop, setRecStop] = useState(false);
  const [playPause, setPlayPause] = useState(false);
  const [isLoadingIn, setIsLoadingIn] = useState(false);
  const [res, setRes] = useState('');
  const [recordSecs, setRecordSecs] = useState(0);
  const [recordTime, setRecordTime] = useState('77:77:77');
  const [currentPositionSec, setCurrentPositionSec] = useState(0);
  const [currentDurationSec, setCurrentDurationSec] = useState(0);
  const [playTime, setPlayTime] = useState('77:77:77');
        ...
        <View style={{width: '90%'}}>
          ...
          <View style={styles.timeView}>
            ...
            <View
              ...
              <TouchableOpacity onPress={ setStartRec(false)}>
