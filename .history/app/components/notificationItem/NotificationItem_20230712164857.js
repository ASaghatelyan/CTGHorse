...
export function NotificationItem({
  ...
  const showItem = async () => {
    // /make-as-read/{id}
    try {
      let res = await axiosInstance.post(`/mark-as-read/${notiId}`);
      navigation.navigate('NewContract', {contract,notiId,senderId});
      getinfo();
      // dispatch({
      //   type: 'SET_USERINFO',
    ...
    <TouchableOpacity style={styles.itemView} onPress={showItem}>
      <View style={styles.leftView}>
        ...
        <View style={styles.textView}>
          <Text>{text}</Text>
          <Text>{moment(date).format('MMMM DD YYYY hh:mm')}</Text>
        </View>
      </View>
      <TouchableOpacity>
