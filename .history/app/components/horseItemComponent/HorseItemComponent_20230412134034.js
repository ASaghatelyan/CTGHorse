...
export const HorseItemComponent = ({item, index, owner, navigation}) => {
  ...
  console.log(item?.photos?.split(',')[0]); 
  return (
    //   <FlatList
    //     data={props.data}
    ...
    <DropShadow
      ...
      <TouchableOpacity
        onPress={() =>
          owner
            ? navigation.navigate('ChangeHorseInfo', {item})
        ...
        <View
          ...
          transition={{delay: 1000 + index * 200}}>
          <View style={styles.imageContainer}>
            <Image source={{uri:item?.photos?.split(',')[0]}} style={styles.image} />
            <Text>dsffbvefwarfsbdx</Text>
            {likeDisLike ? (
              <TouchableOpacity
